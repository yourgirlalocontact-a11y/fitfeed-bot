...
function generateProfileCard({ username, followers, following, avatarImage, photos }) {
  ...
  // --- Username ---
  // Positions calibrées sur la maquette (project/Profile.dc.html) mesurée au
  // pixel près dans un vrai navigateur, plutôt que déduites à la main —
  // évite que la carte générée dérive de la maquette au fil des retouches.
  const usernameY = 219;
  ctx.font = "19px 'Space Grotesk Bold'";
  ctx.fillStyle = COLORS.ink;
  ctx.textAlign = "center";
  const displayUsername = truncateToWidth(ctx, formatHandle(username), WIDTH - 48);
  ctx.fillText(displayUsername, avatarCx, usernameY);
  ctx.textAlign = "left";
